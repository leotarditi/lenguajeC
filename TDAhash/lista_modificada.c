...
void* lista_iterador_siguiente(lista_iterador_t* iterador) {
	...
	return iterador->anterior->elemento;
}
...
int lista_iterador_insertar(lista_iterador_t* iterador, void* elemento) {
	...
	if(lista_vacia(iterador->lista)) {
		iterador->lista->nodo_inicio = nuevo_nodo;
	}
	...
	if(!iterador->actual) {
		iterador->lista->nodo_fin = nuevo_nodo;
	}
...
void* lista_iterador_borrar(lista_iterador_t* iterador) {
	...
	void* elemento = iterador->actual->elemento;
	auxiliar = iterador->actual;
	iterador->actual = iterador->actual->siguiente;
	if(iterador->anterior) {
		iterador->anterior->siguiente = iterador->actual;
	} else {
		iterador->lista->nodo_inicio = iterador->actual;
	}
	if(iterador->actual == NULL) {
		iterador->lista->nodo_fin = iterador->anterior;
	}
	nodo_destruir(auxiliar);
	iterador->lista->cantidad --;
	return elemento;
}
