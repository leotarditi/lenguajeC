...
void guardar_vehiculo(hash_t* garage, const char* patente, const char* descripcion){
  void* elemento = strdup(descripcion);
  int retorno = hash_insertar(garage, patente, elemento);
...
int main(){
	int cantidad_pruebas = 11;
	int cantidad_pruebas_pasadas = 0;
	cantidad_pruebas_pasadas += miniprueba();
	printf("\n\n\n\n");
	cantidad_pruebas_pasadas += DadoUnHashVacioCuandoRealiceLasDiferentesFuncionesDevolveraLoPreestablecidoEnElContrato();
	printf("\n\n\n\n");
	cantidad_pruebas_pasadas += DadoUnHashVacioCuandoItereEseHashDevolveraLoPreestablecidoEnElContrato();
	printf("\n\n\n\n");
	cantidad_pruebas_pasadas += DadaUnaCantidadDeClavesYElementosCuandoInserteDevolveraLosValoresEsperados();
	printf("\n\n\n\n");
	cantidad_pruebas_pasadas += DadaUnaCantidadDeClavesConElmentosCuandoQuieraInsertarLaMismaClaveConDistintoElementoDevolveraLaClaveConElUltimoElementoIngresado();
	printf("\n\n\n\n");
	cantidad_pruebas_pasadas += DadoUnaCantidadDeClavesConElementosAlmacenadosEnMemoriaDinamicaCuandoLosReemplazeYDestruyanDevolveranLosValoresEsperados();
	printf("\n\n\n\n");
	cantidad_pruebas_pasadas += DadaUnaCantidadDeClavesYElementosCuandoBorreDevolveraLosValoresEsperados();
	printf("\n\n\n\n");
	cantidad_pruebas_pasadas += DadaUnaGranCantidadDeElementosCuandoLosInsertoyBorroDeUnHashDevolveraLosValoresEsperados(1000);
	printf("\n\n\n\n");
	cantidad_pruebas_pasadas += DadoUnHashCuandoUseElIteradorDevolveraLosValoresEsperados();
	printf("\n\n\n\n");
	cantidad_pruebas_pasadas += DadoUnHashConMuchosElementosCuandoUseElIteradorDevolveraLosValoresEsperados(2000);
	printf("\n\n\n\n");
	cantidad_pruebas_pasadas += DadoUnHashNULLCuandoRealiceLasDiferentesFuncionesDevolveraLoPreestablecidoEnElContrato();
	printf("\n\n\n\n");
